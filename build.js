...
// ------------------------ HTML/CSS -----------------------------------------
const CSS=`
:root{ --bg:#ffffff; --fg:#0f172a; --muted:#64748b; --card:#f8fafc; --border:#e2e8f0; --accent:#2563eb; --accent-contrast:#ffffff; --ring:rgba(37,99,235,.18); --radius:16px; --maxw:1120px; }
...
body{ margin:0; background:var(--bg); color:var(--fg); font:16px/1.55 'Inter', system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Helvetica, Arial; display:grid; place-items:start center; }
.container{ width:100%; max-width:var(--maxw); padding:32px 20px 48px; }
.header{ position:sticky; top:0; z-index:50; text-align:center; margin-bottom:22px; padding-bottom:10px; background:color-mix(in oklab, var(--bg) 92%, transparent); backdrop-filter:saturate(120%) blur(6px); }
...
.grid{ margin-top:26px; display:grid; grid-template-columns:1fr; gap:12px; }@media (min-width: 1024px){ .grid{ grid-template-columns: 1fr 1fr; gap:14px; } }
.card{ background:var(--card); border:1px solid var(--border); border-radius:var(--radius); padding:12px 14px; display:grid; grid-template-columns:1fr auto; grid-template-rows:auto auto; grid-template-areas:"left badge" "bottom bottom"; row-gap:8px; column-gap:12px; transform:translateY(6px); opacity:.0; transition:transform .25s ease, opacity .25s ease, box-shadow .15s ease, border-color .08s ease; }
.card:hover{ transform:translateY(-1px); box-shadow:0 6px 20px var(--ring), 0 0 0 3px color-mix(in oklab, var(--accent) 25%, transparent); border-color:transparent; }
...
.back-top{ position:fixed; right:16px; bottom:16px; width:40px; height:40px; border-radius:999px; border:1px solid var(--border); background:var(--card); color:var(--fg); display:inline-flex; align-items:center; justify-content:center; cursor:pointer; box-shadow:0 6px 20px var(--ring); opacity:0; pointer-events:none; transition:opacity .2s ease, transform .2s ease; }
...
<script>(function(){
  // Bookmark name during drag/context menu
  var links=document.querySelectorAll('.btn');
  links.forEach(function(a){
    function setName(){a.dataset.originalText=a.textContent;var nm=a.dataset.bmname||a.textContent;a.textContent=nm;a.setAttribute('title','Bookmark name: '+nm);}
    function restore(){if(a.dataset.originalText){a.textContent=a.dataset.originalText;delete a.dataset.originalText;}}
    a.addEventListener('dragstart',setName);
    a.addEventListener('dragend',restore);
    a.addEventListener('contextmenu',function(){setName();setTimeout(restore,1200);});
  });
  // Reveal cards on load
  document.addEventListener('DOMContentLoaded', function(){
    var cards=[].slice.call(document.querySelectorAll('.card'));
    cards.forEach(function(c,i){ c.style.transitionDelay=(i*40)+'ms'; c.style.opacity='1'; c.style.transform='translateY(0)'; });
  });
  // Back-to-top visibility + action
  var bt=document.getElementById('backTop');
  window.addEventListener('scroll', function(){ if(window.scrollY>300){ bt&&bt.classList.add('show'); } else { bt&&bt.classList.remove('show'); } });
  bt && bt.addEventListener('click', function(){ window.scrollTo({top:0, behavior:'smooth'}); });
})();</script>
</body>
</html>`;
